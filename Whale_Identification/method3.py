...
# reference: https://www.kaggle.com/CVxTz/beating-the-baseline-keras-lb-0-38/code
...
from keras.layers import Conv2D, MaxPooling2D, Input, Dense, Flatten, GlobalMaxPooling2D,Dropout,Lambda,merge
...
def get_base_model():
    latent_dim = 50
    base_model = ResNet50(weights= None,include_top=False) # use weights='imagenet' locally
...
def augment(im_array):
 ...
 # 'M:/dataset/Whale_Identification/train5826e16c.jpg'
def gen(triplet_gen):
    while True:
...
history = model.fit_generator(gen(train_gen), validation_data=gen(test_gen), epochs=num_epochs, verbose=1, workers=4, use_multiprocessing=False,
...
train_files = glob.glob(INPUT_DIR+"/train/*.jpg")
test_files = glob.glob(INPUT_DIR+"/test/*.jpg")
